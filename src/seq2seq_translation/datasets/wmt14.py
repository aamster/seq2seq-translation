...
class WMT14(LanguagePairsDataset):
    def __init__(self, out_dir: str | Path, source_lang: str, target_lang: str, split: str):
    ...
    def write_to_single_file(self):
        ...
        with open(self._source_path, 'wb') as source_f:
            with open(self._target_path, 'wb') as target_f:
                for example in tqdm(iter(self._ds), desc=f'Writing WMT14 {self._split} to disk', total=len(self)):
